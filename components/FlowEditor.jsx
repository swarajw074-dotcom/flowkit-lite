...
function Inner({ flowId, initialFlow }) {
  ...
  const onConnect = useCallback(
    (conn) => setEdges((eds) => addEdge({ ...conn, animated: false, style: { stroke: "#999", strokeWidth: 1.5 } }, eds)),
    [setEdges]
  );
  ...
  const addNode = (type) => {
    ...
    const node = { id, type: "default", position, data };
  ...
  const run = async () => {
    ...
    const iv = setInterval(async () => {
      ...
      if (js.status === "completed" || js.status === "failed") clearInterval(iv);
    }, 800);
  };
        ...
        <Panel position="top-left">
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <span style={{ marginLeft: 12, color: "#666" }}>
              Nodes: {nodes.length} {status ? `· ${status}` : ""}
            </span>
          </div>
        </Panel>
      </ReactFlow>
...
export default function FlowEditor(props) {
  ...
  const addNode = (type) => {
    ...
    let data = { label: type };
    if (type === "Webhook") data = { label: "Webhook Trigger" };
    if (type === "HTTP") data = { label: "HTTP Request", method: "GET", url: "https://api.github.com" };
    if (type === "Set") data = { label: "Set Variables", kv: [{ key: "foo", value: "bar" }] };
    if (type === "Delay") data = { label: "Delay (ms)", ms: 1000 };
    if (type === "Code") data = { label: "Code (async JS)", code: "return { now: Date.now() }" };
